...
function buildPrompt(f, images = []) {
  ...
  const secsPrompt = secs.map((s, i) => {
    const n = i + 5;
    const m = {
      services:     `${n}. SERVICES: 6 cards in 3-col grid, icon, name, description, price, hover glow${hasImages?" — image texture in card bg":""}`,
      ...
      gallery:      `${n}. GALLERY: 6-item CSS grid${hasImages?` using: ${[img(0),img(1),img(2),img(3),img(4),img(5)].filter(Boolean).join(", ")}`:" — gradient placeholders"} — object-fit:cover, caption on hover`,
  ...
  return [
    ...
    "1. Hero: Headline words slide up with staggered delay (0.1s). Subheadline fades. CTAs pop with scale.",
    "2. Scroll reveal: Every section fades up with translateY(40px) via IntersectionObserver. Stagger grid children 0.1s.",
    "3. Stats counter: Smooth count-up from 0 over 2.5s with easing on scroll into view.",
    "4. Service cards: Hover — translateY(-10px), deeper shadow, accent top border, 0.3s transition.",
    "5. Nav: Transparent on hero, solid + backdrop-blur after 80px scroll. Active section highlights.",
    "6. Images: Scale 1.0 to 1.05 on hover with overflow:hidden. Fade-in on load.",
    "7. CTA buttons: Ripple on click. Glow pulse (box-shadow) on primary button.",
    "8. Section headings: Underline grows from left on scroll-in via CSS scaleX.",
    "9. Testimonials: Slide in from alternating sides.",
    "10. Mobile menu: Slides down smoothly.",
    "Use ONLY CSS @keyframes and vanilla JS — no libraries.",
    "",
    "RULES: CSS in <style>, JS in <script> at end. Real niche copy, no lorem ipsum. 5+ CTAs. Working FAQ accordion and hamburger. Mobile-first responsive. All images object-fit:cover.",
    "",
