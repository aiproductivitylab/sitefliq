...
function buildPrompt(f, images = []) {
  ...
  return [
    ...
    mapsHtml,
    "",
"DESIGN QUALITY — THIS IS CRITICAL. The page must look like it was built by a professional agency:",
"- Service cards: each card must have border-radius:16px, box-shadow:0 4px 24px rgba(0,0,0,.08), padding:32px, background:var(--surface) or white, with an icon div (60x60px, colored background circle), bold service name (20px+), description text, price badge, and smooth hover transform. Display in CSS grid 3 columns.",
"- Testimonial cards: each must have a colored top border (4px accent color), star rating in accent color, italic quote text, and an avatar circle with initials. Display in CSS grid 3 columns with gap:24px.",
"- Gallery: MUST use a different image for each of the 6 items. Display as CSS grid 3x2, each item height:280px, overflow:hidden, position:relative, with a hover overlay div showing caption. Each img must be 100% width and height, object-fit:cover.",
"- Stats counter: each stat number must use data-target attribute and be animated with JS counting from 0. Stats bar must have a colored background (accent color), display as 4-column grid.",
"- Section headings: font-size at least 42px, font-family from Google Fonts (Oswald, Playfair Display, or similar bold font), with a colored underline accent below.",
...
"",
"RULES: CSS in <style>, JS in <script> at end. Real niche copy, no lorem ipsum. 5+ CTAs. Working FAQ accordion and hamburger. All images object-fit:cover.",
"",
